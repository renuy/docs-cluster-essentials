# Deploying Cluster Essentials v1.8
...
1.  Select a download according to your Kubernetes provider and operating system:
    ...
    - For macOS, download `tanzu-cluster-essentials-darwin-amd64-1.8.0.tgz`.
    - For Linux, download `tanzu-cluster-essentials-linux-amd64-1.8.0.tgz`.
    - For Windows, download `tanzu-cluster-essentials-windows-amd64-1.8.0.tgz`.
...
1. For air-gapped installation, download the bundle:
    ...
    $ IMGPKG_REGISTRY_HOSTNAME=registry.tanzu.vmware.com \
      ...
      ./imgpkg copy \
        ...
        --to-tar cluster-essentials-bundle-1.8.0.tar \
...
- For air-gapped installation: 
    ...
    $ IMGPKG_REGISTRY_HOSTNAME=MY-REGISTRY \
      ...
      ./imgpkg copy \
        --tar cluster-essentials-bundle-1.8.0.tar \
...
For all other clusters, if you already have Cluster Essentials 1.0+ installed on your target cluster, you can upgrade to Cluster Essentials 1.8 using the following steps. Running this upgrade will update the `kapp-controller` version on your cluster to `v0.48.1` and `secretgen-controller` version to `v0.15.0`.
