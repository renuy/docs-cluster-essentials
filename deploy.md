...
This topic tells you how to install, upgrade, and uninstall Cluster Essentials v1.4.2.
...
1.  Select a download according to your Kubernetes provider and operating system:
    ...
    - For macOS, download `tanzu-cluster-essentials-darwin-amd64-1.4.2.tgz`.
    - For Linux, download `tanzu-cluster-essentials-linux-amd64-1.4.2.tgz`.
    - For Windows, download `tanzu-cluster-essentials-windows-amd64-1.4.2.tgz`.
...
1. Configure and run `install.sh`, which will install `kapp-controller` and `secretgen-controller` on your cluster:
    ...
    - For air-gapped installation, run:
        ...
        $ IMGPKG_REGISTRY_HOSTNAME=MY-REGISTRY \
          ...
          imgpkg copy \
            --tar cluster-essentials-bundle-1.4.2.tar \
            --to-repo MY-REGISTRY/cluster-essentials-bundle \
            --include-non-distributable-layers \
            --registry-ca-cert-path CA_PATH

        $ INSTALL_BUNDLE=MY-REGISTRY/cluster-essentials-bundle@sha256:a119cb90111379a5f91d27ae572a0de860dd7322179ab856fb32c45be95d78f5 \
...
1. Configure and run `install.bat`, which will install `kapp-controller` and `secretgen-controller` on your cluster:
    ...
    - For online installation, run:
        ...
        set INSTALL_BUNDLE=registry.tanzu.vmware.com/tanzu-cluster-essentials/cluster-essentials-bundle@sha256:a119cb90111379a5f91d27ae572a0de860dd7322179ab856fb32c45be95d78f5
    ...
    - For air-gapped installation, run:
        ...
        set INSTALL_BUNDLE=MY-REGISTRY/cluster-essentials-bundle@sha256:a119cb90111379a5f91d27ae572a0de860dd7322179ab856fb32c45be95d78f5
...
For all other clusters, if you already have Cluster Essentials 1.0+ installed on your target cluster, you can upgrade to Cluster Essentials 1.4.2 using the following steps. Running this upgrade will update the `kapp-controller` version on your cluster to `v0.44.8` and `secretgen-controller` version to `v0.13.3`.
