# Deploying Cluster Essentials v1.3

This topic tells you how to install, upgrade, and uninstall Cluster Essentials v1.3.1.
...
1.  Select a download according to your Kubernetes provider and operating system:
    ...
    - For macOS, download `tanzu-cluster-essentials-darwin-amd64-1.3.1.tgz`.
    - For Linux, download `tanzu-cluster-essentials-linux-amd64-1.3.1.tgz`.
    - For Windows, download `tanzu-cluster-essentials-windows-amd64-1.3.1.tgz`.
...
1. Configure and run `install.sh`, which will install `kapp-controller` and `secretgen-controller` on your cluster:
    ...
    - For air-gapped installation, run:
        ...
        $ IMGPKG_REGISTRY_HOSTNAME=MY-REGISTRY \
          ...
          imgpkg copy \
            --tar cluster-essentials-bundle-1.3.1.tar \
...
For all other clusters, if you already have Cluster Essentials 1.0+ installed on your target cluster, you can upgrade to Cluster Essentials 1.3 using the following steps. Running this upgrade will update the `kapp-controller` version on your cluster to `v0.41.8` and `secretgen-controller` version to `v0.11.6`.
